...
export const ROUTING_RULES: string[] = [
  ...
  'Compound message (multiple distinct asks: “and”, “also”, lists, semicolons) → extract every card; emit all operations[] with distinct entity_id per add; prefer explicit ordered canvas_ops[] (order becomes preview review queue). Pick dominant intent_type (usually add_feature for multi-add). Cross-type compounds allowed under one plan.',
]
