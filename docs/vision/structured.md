...
**Agents:** maintain docs per [meta/documentation-guide.md](./meta/documentation-guide.md).  
**Current build focus:** Flow B web app — intent canvas + orchestration.  
**Live status:** [implementation/current-state.md](./implementation/current-state.md)

```
docs/
  README.md                  ← Doc index (start here)
  meta/
    documentation-guide.md   ← How agents update docs; USER vs AI-INFERRED
    user-decisions.md        ← [USER] explicit decisions
    ai-inferred-decisions.md ← [AI-INFERRED] agent-chosen defaults
  implementation/
    current-state.md         ← Built & deployed now
  vision/
    original.md
    structured.md            ← This file
  architecture/
    translator-platform.md
    translator-internals.md
    flow-b-v0.md
    flow-a-v0.md             ← Deferred
  product/
    canvas-ui-vision.md
    workflow.md
    tech-stack.md
    execution-phases.md      ← Future

frontend/                    ← React canvas app [built]
insforge/functions/nlide-api ← Edge translator API [deployed, stub]
migrations/                  ← Postgres schema [applied]
spec/                        ← Exported intent MD on commit [not yet]
AGENTS.md                    ← InsForge SDK + doc rules
```
