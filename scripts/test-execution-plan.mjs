...
// Dynamic import via ts not available — inline minimal checks using read tasks.md
...
function validate(plan, tasks) {
  ...
  for (const taskId of expected) {
    if (!assigned.has(taskId)) issues.push(`missing ${taskId}`)
  }
  return issues
}

// Stub-like plan from tasks by pillar
...
const okIssues = validate(plan, tasksMd)
if (okIssues.length > 0) {
  console.error('FAIL valid plan:', okIssues)
  process.exit(1)
}
...
const badIssues = validate(badPlan, tasksMd)
if (badIssues.length === 0 || !badIssues.some((i) => i.startsWith('missing'))) {
  console.error('FAIL expected missing task errors')
  process.exit(1)
}
